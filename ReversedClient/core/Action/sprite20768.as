...
if (!dofus.datacenter.Spell)
{
    ...
    var _loc1 = (_global.dofus.datacenter.Spell = function (nID, nLevel, sCompressedPosition, nBoundObjectId)
    {
        super();
        this.initialize(nID, nLevel, sCompressedPosition, nBoundObjectId);
    }).prototype;
    ...
    _loc1.__set__boundObjectId = function (nBoundObjectId)
    {
        this._nBoundObjectId = nBoundObjectId;
        //return (this.boundObjectId());
    };
    _loc1.__get__boundObjectId = function ()
    {
        return (this._nBoundObjectId);
    };
    _loc1.__set__position = function (nPosition)
    {
        if (nPosition != this._nPosition)
        {
            this.api.network.Spells.moveToUsed(this.ID, nPosition);
        } // end if
    ...
    _loc1.initialize = function (nID, nLevel, sCompressedPosition, nBoundObjectId)
    {
        this.api = _global.API;
        this._nID = nID;
        this._nLevel = nLevel;
        this._nBoundObjectId = nBoundObjectId == undefined ? (null) : (nBoundObjectId);
        ...
        var _loc6 = this.getSpellLevelText(15);
        var _loc7 = _loc6.split("");
        this._aEffectZones = new Array();
        var _loc8 = 0;
        
        while (_loc8 = _loc8 + 2, _loc8 < _loc7.length)
        {
            this._aEffectZones.push({shape: _loc7[_loc8], size: ank.utils.Compressor.decode64(_loc7[_loc8 + 1])});
        } // end while
        this._bSummonSpell = this.searchIfSummon(this.getSpellLevelText(0)) || this.searchIfSummon(this.getSpellLevelText(1));
        this._nMaxLevel = 1;
        var _loc9 = 1;
        
        while (++_loc9, _loc9 <= dofus.Constants.SPELL_BOOST_MAX_LEVEL)
        {
            if (this._oSpellText["l" + _loc9] == undefined)
            {
                break;
                continue;
            } // end if
            this._nMaxLevel = _loc9;
        } // end while
    ...
    _loc1.addProperty("boundObjectId", _loc1.__get__boundObjectId, _loc1.__set__boundObjectId);
