...
if (!com.ankamagames.tools.Logger)
{
    ...
    var _LoggerProto = (_global.com.ankamagames.tools.Logger = function ()
    {
    ...
    }).out = function ()
    {
        ...
        var level = arguments.length > 4 ? (arguments[1]) : (undefined);
        var message = new String(msg);
        if (message.substr(0, 2) == "WG")
        {
            message = message.substr(2);
            level = com.ankamagames.tools.Logger.WG;
        }
        else if (message.toUpperCase().indexOf("[EXCEPTION]") == 0)
        {
            message = message.substr(12);
            level = com.ankamagames.tools.Logger.ERROR;
        }
        else if (message.toUpperCase().indexOf("[WTF]") == 0)
        {
        ...
        {
            message = message.substr(5);
            level = com.ankamagames.tools.Logger.FATAL;
        } // end else if
        com.ankamagames.tools.Logger.instance.trace(message, level);
    };
    _LoggerProto.connect = function ()
    {
    ...
    _LoggerProto.trace = function (message, level)
    {
        if (level == undefined)
        {
            level = com.ankamagames.tools.Logger.TRACE;
    ...
    _LoggerProto.onSocketClose = function ()
    {
    ...
    _LoggerProto.onConnected = function (success)
    {
        var _loc3 = 0;
    ...
    ASSetPropFlags(_LoggerProto, null, 1);
    (_global.com.ankamagames.tools.Logger = function ()
    {
        this._buffer = new Array();
        this.connect();
    ...
    _LoggerProto._nCurrentReconnection = 0;
} // end if
#endinitclip
