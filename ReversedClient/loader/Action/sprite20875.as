        ...
        switch (this._sType)
        {
            case "book":
            {
                ...
                if (_loc10 != 0)
                {
                    ...
                    while (++_loc11, _loc11 < _loc10)
                    {
                        var _loc12 = new String();
                        if (_loc4[_loc11] != undefined)
                        {
                            if (_loc4[_loc11][2] && _loc3 % 2 == 0)
                            {
                                this._aPages.push({type: "blank", num: _loc3++});
                            } // end if
                            _loc4[_loc11][4] = _loc3;
                            if (_loc4[_loc11][3])
                            {
                                _loc12 = "<br/><p class=\'chapter\'>" + _loc4[_loc11][0] + "</p><br/>";
                            } // end if
                        } // end if
                        _loc9[_loc11] = ank.utils.PatternDecoder.replaceStr(_loc9[_loc11], "##local,", "##" + dofus.Constants.CLIPS_PATH + ",");
                        _loc9[_loc11] = ank.utils.PatternDecoder.replaceStr(_loc9[_loc11], "##server,", "##" + this.api.kernel.DocumentsServersManager.getCurrentServer() + ",");
                        _loc9[_loc11] = ank.utils.PatternDecoder.replace(_loc9[_loc11], "#1#2");
                        this._aPages.push({type: "text", num: _loc3++, text: _loc12 + _loc9[_loc11], cssFile: this._sCSS});
                    } // end while
                } // end if
                break;
            } 
            ...
            case "roadsignright":
            {
                var _loc13 = mcSwfData.pages[0];
                var _loc14 = this.api.kernel.DocumentsServersManager.getCurrentServer() + "#1/#2.#1";
                _loc13 = ank.utils.PatternDecoder.replace(_loc13, _loc14);
                this._aPages.push({text: _loc13, cssFile: this._sCSS});
                break;
            } 
        } // End of switch
    };
