...
if (!dofus.graphics.gapi.ui.StatsJob)
{
    ...
    _loc1.addListeners = function ()
    {
        ...
        this._cbTitle.addEventListener("itemSelected", this);
    };
    _loc1.initData = function ()
    {
        ...
        this._winBg.title = this.api.datacenter.Player.data.name;
        this.activateBoostButtons(!this.api.datacenter.Game.isFight);
        this._eatitles = new ank.utils.ExtendedArray();
        this._eatitles.push({label: "None", index: 0, id: -1});
        this._eaAlltitles = new ank.utils.ExtendedArray();
        var _loc11 = this.api.lang.getAllTitle();
        for (var i in _loc11)
        {
            this._eaAlltitles[i] = {label: _loc11[i].t, index: i, id: i};
        } // end of for...in
        this.getTitleList();
        this._cbTitle.dataProvider = this._eatitles;
    };
    ...
    {
        this._strLPValue = String(oEvent.value);
        this.lpLabelChanged();
    };
    _loc1.lpMaxChanged = function (oEvent)
    {
        this._strLPMaxValue = String(oEvent.value);
        this.lpLabelChanged();
    };
    _loc1.lpLabelChanged = function ()
    {
        this._lblLPValue.text = this._strLPValue + "/" + this._strLPMaxValue;
    };
    ...
    _loc1.itemSelected = function (oEvent)
    {
        switch (oEvent.target._name)
        {
            case "_cbTitle":
            {
                ...
                this.sendTitleChoice(_loc3.id);
                break;
            } 
        } // End of switch
    };
    _loc1.getTitleList = function ()
    {
        this.api.network.send("wTL");
    };
    _loc1.sendTitleChoice = function (index)
    {
        this.api.network.send("wTC" + index);
    };
    _loc1.onTitleList = function (s)
    {
        var _loc3 = s.split(";");
        var _loc4 = _loc3[0];
        var _loc5 = _loc3[1].split("_");
        var _loc6 = 0;
        
        while (++_loc6, _loc6 < _loc5.length)
        {
            this._eatitles[_loc6 + 1] = this._eaAlltitles[_loc5[_loc6]];
            this._eatitles[_loc6 + 1].index = _loc6 + 1;
        } // end while
        this._cbTitle.dataProvider = this._eatitles;
        this.selectTitle(_loc4);
    };
    _loc1.selectTitle = function (title)
    {
        var _loc3 = 0;
        var _loc4 = 0;
        
        while (++_loc4, _loc4 < this._eatitles.length)
        {
            if (this._eatitles[_loc4].id == title)
            {
                _loc3 = this._eatitles[_loc4].index;
                break;
            } // end if
        } // end while
        this._cbTitle.selectedIndex = _loc3;
    };
