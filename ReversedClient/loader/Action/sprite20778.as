...
if (!dofus.aks.Account)
{
    ...
    _loc1.onCharactersList = function (bSuccess, sExtraData, bIsMigration, bIsFromGame)
    {
        this.api.ui.unloadUIComponent("WaitingMessage");
        this.api.ui.unloadUIComponent("WaitingQueue");
        if (bIsFromGame && (this.api.datacenter.Basics.hasSelectedCharacter && this.api.datacenter.Basics.aks_gifts_stack.length == 0))
        {
        ...
        if (bIsFromGame && this.api.datacenter.Basics.isDeletingCharacter)
        {
        ...
        var _loc6 = 0;
        if (bIsFromGame && (this.api.datacenter.Basics.isCreatingNewCharacter && this.api.datacenter.Basics.globalCharacterCount < dofus.Constants.WG_MAX_CHARACTERS_PER_ACCOUNT))
        {
        ...
        com.ankamagames.tools.Logger.out("WG on a les datas " + sExtraData + " à splitter en " + _loc8.length, "dofus.aks.Account::onCharactersList", "E:\\My_Work\\WLG2\\WG2\\client\\DofusClient\\classes/dofus/aks/Account.as", 773);
        var _loc9 = -1;
        if (_loc8[0].split(";").length < 2)
        {
            _loc6 = 1;
            _loc9 = _loc8[0];
        } // end if
        if (!bIsFromGame && _loc9 != -1)
        {
            this.api.datacenter.Basics.globalCharacterCount = _loc9;
            com.ankamagames.tools.Logger.out("WG on a " + _loc9 + " persos", "dofus.aks.Account::onCharactersList", "E:\\My_Work\\WLG2\\WG2\\client\\DofusClient\\classes/dofus/aks/Account.as", 782);
        } // end if
        var _loc10 = new Array();
        this.api.datacenter.Sprites.clear();
        var _loc11 = _loc6;
        
        while (++_loc11, _loc11 < _loc8.length)
        {
            var _loc12 = _loc8[_loc11].split(";");
            var _loc13 = new Object();
            var _loc14 = _loc12[0];
            var _loc15 = _loc12[1];
            _loc13.level = _loc12[2];
            _loc13.gfxID = _loc12[3];
            _loc13.color1 = _loc12[4];
            _loc13.color2 = _loc12[5];
            _loc13.color3 = _loc12[6];
            _loc13.color4 = _loc12[7];
            _loc13.color5 = _loc12[8];
            _loc13.accessories = _loc12[9];
            _loc13.merchant = _loc12[10];
            _loc13.serverID = _loc12[11];
            _loc13.isDead = _loc12[12];
            _loc13.deathCount = _loc12[13];
            _loc13.lvlMax = _loc12[14];
            var _loc16 = this.api.kernel.CharactersManager.createCharacter(_loc14, _loc15, _loc13);
            _loc16.sortID = Number(_loc14);
            _loc7.push(_loc16);
            _loc10.push(Number(_loc14));
        } // end while
        _loc7.sortOn("sortID", Array.NUMERIC);
        this.api.ui.unloadUIComponent("ChooseCharacter");
        this.api.ui.unloadUIComponent("CreateCharacter");
        this.api.ui.unloadUIComponent("ChooseServer");
        this.api.ui.unloadUIComponent("AutomaticServer");
        ...
        if (this.api.datacenter.Basics.hasCreatedCharacter)
        {
            ...
            if (this.api.datacenter.Basics.oldCharList == undefined && _loc10.length == 1 || this.api.datacenter.Basics.oldCharList.length + 1 == _loc10.length)
            {
                var _loc17 = 0;
                
                while (++_loc17, _loc17 < _loc10.length)
                {
                    var _loc18 = false;
                    var _loc19 = 0;
                    
                    while (++_loc19, _loc19 < this.api.datacenter.Basics.oldCharList.length)
                    {
                        if (_loc10[_loc17] == this.api.datacenter.Basics.oldCharList[_loc19])
                        {
                            _loc18 = true;
                            break;
                        } // end if
                    ...
                    if (!_loc18)
                    {
                        this.setCharacter(_loc10[_loc17]);
                        return;
                    } // end if
                } // end while
            } // end if
        } // end if
        this.api.datacenter.Basics.oldCharList = _loc10;
        ...
        else
        {
            ...
            if (_loc9 > 0 && (!bIsMigration || this.api.datacenter.Basics.ignoreMigration))
            {
                this.api.datacenter.Basics.createCharacter = false;
                com.ankamagames.tools.Logger.out("WG on affiches les persos " + _loc7, "dofus.aks.Account::onCharactersList", "E:\\My_Work\\WLG2\\WG2\\client\\DofusClient\\classes/dofus/aks/Account.as", 902);
                this.api.ui.loadUIComponent("ChooseCharacter", "ChooseCharacter", {spriteList: _loc7, remainingTime: 0, characterCount: _loc9}, {bForceLoad: true});
            }
            else if (_loc9 == 0)
            {
                this.api.datacenter.Basics.globalCharacterCount = _loc9;
            ...
            else
            {
                ...
                this.api.datacenter.Basics.createCharacter = false;
                this.api.ui.loadUIComponent("CharactersMigration", "CharactersMigration", {spriteList: _loc7, remainingTime: 0, characterCount: _loc7.length}, {bForceLoad: true});
            } // end else if
        } // end else if
        if (this.api.datacenter.Basics.aks_gifts_stack.length != 0 && _loc7.length > 0)
        {
            this.api.ui.getUIComponent("CreateCharacter")._visible = false;
            this.api.ui.getUIComponent("ChooseCharacter")._visible = false;
            this.api.ui.loadUIComponent("Gifts", "Gifts", {gift: this.api.datacenter.Basics.aks_gifts_stack.shift(), spriteList: _loc7}, {bForceLoad: true});
        } // end if
    };
    ...
    _loc1.onStats = function (sExtraData)
    {
        ...
        _loc4.setXP(_loc5[0], _loc5[1], _loc5[2]);
        _loc4.Kama = _loc3[1];
